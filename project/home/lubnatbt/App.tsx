...
function calcGrind({ prod, gas, cash, rate, boostMult, refCap, targetCostB }) {
  const p = prod,
    g = gas,
    cv = cash;
...
function useSaved(key, defaultValue) {
...
export default function App() {
  ...
  const [inventory, setInventory] = useSaved("inv", makeEmptyInventory());
  const [customTargets, setCustomTargets] = useSaved("customTgts", []);
  ...
  const updateInventory = (plot, type, machine, delta) => {
    setInventory((prev) => {
      const current = prev[plot][type][machine] || 0;
      const next = Math.max(0, current + delta);
      return {
        ...prev,
        [plot]: {
          ...prev[plot],
          [type]: { ...prev[plot][type], [machine]: next },
        },
      };
    });
  };
  ...
  const counterBtn = (onClick, label) => (
    <button
      onClick={onClick}
      style={{
        width: "28px",
